...
export const ThemeToggle = () => {
  ...
  // On ne rend le toggle qu'après montage client
  useEffect(() => {
    setMounted(true);
  }, []);

  if (!mounted) {
    // Rendu neutre côté serveur et pendant hydratation
    return <button aria-label="Toggle theme" className="w-10 h-10 rounded-full bg-gray-300" disabled />;
  }
